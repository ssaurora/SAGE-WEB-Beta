...
import type { SceneAuditEvent, SceneAuditEventLevel, SceneAuditEventType, SceneAuditPageViewModel } from '@/lib/mock/scene';
    ...
    <div className="space-y-4">
      <Card>
        ...
        <CardContent className="grid gap-3 sm:grid-cols-4">
          <div className="rounded-md border p-3">
            ...
            <p className="mt-1 text-sm font-semibold">{vm.summary.totalEvents}</p>
          </div>
          <div className="rounded-md border p-3">
            <p className="text-xs text-muted-foreground">Warnings</p>
            <p className="mt-1 text-sm font-semibold">{vm.summary.warningCount}</p>
          </div>
          <div className="rounded-md border p-3">
            <p className="text-xs text-muted-foreground">Errors</p>
            <p className="mt-1 text-sm font-semibold">{vm.summary.errorCount}</p>
          </div>
          <div className="rounded-md border p-3">
            <p className="text-xs text-muted-foreground">Latest</p>
            <p className="mt-1 text-sm font-semibold">{vm.summary.latestMessage}</p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="flex flex-wrap gap-2">
            ...
            <Button size="sm" variant={levelFilter === 'warning' ? 'default' : 'outline'} onClick={() => setLevelFilter('warning')}>
              警告
            </Button>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button size="sm" variant={typeFilter === 'input' ? 'default' : 'outline'} onClick={() => setTypeFilter('input')}>
              输入
            </Button>
            <Button size="sm" variant={typeFilter === 'runtime' ? 'default' : 'outline'} onClick={() => setTypeFilter('runtime')}>
              运行
            </Button>
            <Button size="sm" variant={typeFilter === 'manifest' ? 'default' : 'outline'} onClick={() => setTypeFilter('manifest')}>
              清单
            </Button>
            <Button size="sm" variant={typeFilter === 'result' ? 'default' : 'outline'} onClick={() => setTypeFilter('result')}>
              结果
            </Button>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>当前筛选结果：{filteredEvents.length} 条</CardDescription>
        </CardHeader>
